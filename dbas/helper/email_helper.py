...
def send_mail_due_to_edit_text(statement_uid, current_author, previous_author, url, request):
    """
    ...
    db_previous_author = DBDiscussionSession.query(User).filter_by(uid=previous_author.author_uid).first()
    db_current_editor = DBDiscussionSession.query(User).filter_by(uid=current_author.author_uid).first()
    ...
    body = TextGenerator.get_text_for_edit_text_message(db_language.ui_locales, db_previous_author.publick_nickname,
                                                        db_textversion_old.content, db_textversion_new.content, url)
    recipient = db_current_editor.email
