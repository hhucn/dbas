...
def __get_bubble_from_justify_statement_step(step, nickname, lang, url):
    ...
    bubble_user = create_speechbubble_dict(BubbleTypes.is_user, message=msg, omit_url=False, statement_uid=uid,
...
def __get_bubble_from_support_step(uid_user, uid_system, nickname, lang, application_url):
    ...
    bubble_user = create_speechbubble_dict(BubbleTypes.is_user, message=user_text, omit_url=True, argument_uid=uid_user,
    ...
    bubble_system = create_speechbubble_dict(BubbleTypes.is_system, message=text, omit_url=True, lang=lang)
...
def __get_bubble_from_attitude_step(step, nickname, lang, url):
    ...
    bubble = create_speechbubble_dict(BubbleTypes.is_user, message=text, omit_url=False, statement_uid=uid, nickname=nickname,
                                      lang=lang, url=url)
...
def __get_bubble_from_dont_know_step(step, nickname, lang, url):
    ...
    sys_bubble = create_speechbubble_dict(BubbleTypes.is_system, message=sys_text, nickname=nickname)

    text = _tn.get(_.showMeAnArgumentFor) + (' ' if lang == 'de' else ': ') + get_text_for_conclusion(db_argument)
    user_bubble = create_speechbubble_dict(BubbleTypes.is_user, message=text, nickname=nickname)
...
def get_bubble_from_reaction_step(main_page, step, nickname, lang, splitted_history, url, color_steps=False):
    ...
    bubble_user = create_speechbubble_dict(BubbleTypes.is_user, message=user_text, omit_url=False, argument_uid=uid,
                                           is_supportive=is_supportive, nickname=nickname, lang=lang, url=url)
    if attack == 'end':
        bubble_syst = create_speechbubble_dict(BubbleTypes.is_system, message=sys_text, omit_url=True, nickname=nickname,
                                               lang=lang)
    else:
        bubble_syst = create_speechbubble_dict(BubbleTypes.is_system, id='question-bubble-' + str(additional_uid),
