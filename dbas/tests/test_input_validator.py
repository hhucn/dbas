...
class InputValidatorTests(unittest.TestCase):
    ...
    def test_check_reaction(self):
        ...
        self.assertEqual(reaction.check_reaction(attacked_arg_uid=1,
                                                 attacking_arg_uid=1,
                                                 relation='end',
    ...
    def main_discussion(self, argv=sys.argv):
        if len(argv) != 2:
            self.usage(argv)
        config_uri = sys.argv[1]
        ...
        with transaction.manager:
            user2 = self.set_up_users(DBDiscussionSession)
            self.setup_discussion_database(DBDiscussionSession, user2)
            transaction.commit()


    def main_discussion_reload(self, argv=sys.argv):
        if len(argv) != 2:
            self.usage(argv)
        config_uri = argv[1]
        setup_logging(config_uri)
        ...
        with transaction.manager:
            ...
            self.setup_discussion_database(DBDiscussionSession, main_author)
            self.setup_dummy_votes(DBDiscussionSession)
            transaction.commit()


    def main_dummy_votes(self, argv=sys.argv):
        if len(argv) != 2:
            self.usage(argv)
        config_uri = argv[1]
        setup_logging(config_uri)
