...
from dbas import string_matcher, DBDiscussionSession
...
class StringMatcherTest(unittest.TestCase):
    string_matcher.mechanism = 'Levensthein'

    def test_get_strings_for_start(self):
        mechanism, return_array = string_matcher.get_strings_for_start('cat', 2, True)
        self.check_string_matcher_array(return_array)

        mechanism, return_array = string_matcher.get_strings_for_start('cat', 2, False)
        self.check_string_matcher_array(return_array)

    def test_get_strings_for_reasons(self):
        mechanism, return_array = string_matcher.get_strings_for_reasons('cat', 2)
        self.check_string_matcher_array(return_array)

    def test_get_strings_for_issues(self):
        mechanism, return_array = string_matcher.get_strings_for_issues('cat')
        ...
        mechanism, return_array = string_matcher.get_strings_for_issues('ccaatt')
        count = 0
        for entry in return_array:
    ...
    def test_get_strings_for_search(self):
        return_dict = string_matcher.get_strings_for_search('cat')
        for key in return_dict:
            self.assertTrue('cat' in return_dict[key]['text'].lower())
            self.assertGreater(return_dict[key]['statement'], 0)

    def test_get_distance(self):
        self.assertEqual(3, int(string_matcher.get_distance('cat', 'dog')))
        self.assertEqual(2, int(string_matcher.get_distance('cat', 'dag')))
        self.assertEqual(2, int(string_matcher.get_distance('cat', 'doT')))
