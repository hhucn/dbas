...
class TestReviewFlagHelper(unittest.TestCase):
    ...
    def test_flag_argument(self):
        success, info, error = rf_helper.flag_argument(0, 'reason', self.some_nick, True)
        self.__assert_equal_text([[success, ''], [info, ''], [error, _.internalKeyError]])

        success, info, error = rf_helper.flag_argument(4, 'reason', self.some_nick, True)
        self.__assert_equal_text([[success, ''], [info, ''], [error, _.internalKeyError]])

        success, info, error = rf_helper.flag_argument(4, 'reason', self.tobias, True)
        self.__assert_equal_text([[success, ''], [info, ''], [error, _.internalKeyError]])

        success, info, error = rf_helper.flag_argument(0, 'reason', self.tobias, True)
        self.__assert_equal_text([[success, ''], [info, ''], [error, _.internalKeyError]])

        success, info, error = rf_helper.flag_argument(4, 'optimization', self.tobias, True)
        ...
        success, info, error = rf_helper.flag_argument(4, 'optimization', self.christian, True)
