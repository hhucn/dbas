...
class Translator(object):
	...
	def __init__(self, lang):
		...
		self.andIDoBelieveCounter = 'andIDoBelieveCounter'
	...
	def set_up_en_dict(self):
		...
		en_lang[self.andIDoBelieveCounter] = 'and I do believe that this is a good counter-argument for'
		en_lang[self.andIDoBelieveArgument] = 'and I do believe that this is a good argument for'
	...
	def set_up_de_dict(self):
		...
		de_lang[self.andIDoBelieveCounter] = 'und ich glaube, dass ist ein gutes Gegenargument für',
...
class TextGenerator(object):
	...
	def get_text_for_add_premise_container(self, confrontation, premise, attack_type, conclusion, is_supportive):
		...
		if attack_type == 'overbid':
			ret_text = confrontation + ', ' + _t.get(_t.andIDoBelieveCounter) + ' ' + conclusion
	...
	def get_header_for_confrontation_response(self, confrontation, premise, attack_type, conclusion, start_lower_case, is_supportive, is_logged_in):
		"""
		Based on the users reaction, text will be build.
		:param confrontation: choosen confrontation
		:param premise: current premise
		...
		if attack_type == 'undercut':
			...
			ret_text += _t.get(_t.butIDoNotBelieveCounter) if is_supportive else _t.get(_t.butIDoNotBelieveArgument)
			ret_text += ' <strong>' + conclusion + '</strong>.'

		if attack_type == 'overbid':
			ret_text += r + '<strong>' + premise + '</strong>, '
			ret_text += _t.get(_t.andIDoBelieveCounter) if is_supportive else _t.get(_t.andIDoBelieveArgument)
			ret_text += ' <strong>' + conclusion + '</strong>.<br><br>'
			ret_text += _t.get(_t.howeverIHaveEvenStrongerArgumentAccepting) if is_supportive else _t.get(_t.howeverIHaveEvenStrongerArgumentRejecting)
			ret_text += ' <strong>' + conclusion + '</strong>.'

		if attack_type == 'rebut':
			ret_text += r + '<strong>' + premise + '</strong>, '
			ret_text += _t.get(_t.iAcceptCounter) if is_supportive else _t.get(_t.iAcceptArgument)
			ret_text += ' <strong>' + conclusion + '</strong>.<br><br>'
			ret_text += _t.get(_t.howeverIHaveMuchStrongerArgumentRejecting) if is_supportive else _t.get(_t.howeverIHaveMuchStrongerArgumentAccepting)
			ret_text += ' <strong>' + premise + '</strong>.'
	...
	def get_confrontation_relation_text_dict(self, confrontation, conclusion, premise, start_lower_case, is_supportive):
		"""
		...
		if is_supportive:
			counter_justification = ' <strong>' + conclusion + ', ' + _t.get(_t.because).lower() + ' ' + premise + '</strong>'
		else:
			counter_justification = ' <strong>' + premise + ', ' + _t.get(_t.doesNotJustify).lower() + ' ' + conclusion + '</strong>'
		...
		undercut  = r + ', <strong>' + confrontation + '</strong>, ' + _t.get(_t.butIDoNotBelieveCounter) + ' ' + counter_justification + '.'
		overbid	  = r + ', <strong>' + confrontation + '</strong>, ' + _t.get(_t.andIDoBelieveCounter) + ' ' + counter_justification + '.<br>' + \
		               (_t.get(_t.howeverIHaveEvenStrongerArgumentAccepting) if is_supportive else
	...
	def get_relation_text_dict_without_confrontation(self, premises, conclusion, start_lower_case, with_no_opinion_text, is_attacking):
		"""
		...
		ret_dict['overbid_text'] = r + ', <strong>' + premise + '</strong>, ' + _t.get(_t.andIDoBelieveCounter) + ' <strong>' + conclusion + '</strong>. ' + \
	...
	def get_text_dict_for_attacks_only(self, premises, conclusion, start_lower_case):
		"""
	...
	def get_text_for_confrontation(self, premise, conclusion, supportive, attack, confrontation, reply_for_argument,
		...
		# build some confrontation text
		if attack == 'undermine':
			confrontation_text = _t.get(_t.otherParticipantsThinkThat) + ' <strong>' + premise + '</strong> ' \
			...
			if reply_for_argument:  # reply for argument
				...
				confrontation_text = _t.get(_t.otherParticipantsAgreeThat) + ' ' + _t.get(_t.strongerStatementForRecjecting)
			...
			confrontation_text = _t.get(_t.otherParticipantsAgreeThat) + ' <strong>' + premise + '</strong> ' \
								 + (_t.get(_t.butTheyDoNotBelieveCounter)
								   if supportive else _t.get(_t.butTheyDoNotBelieveArgument)) \
								 + ' <strong>' + conclusion + '</strong>,' + ' ' + _t.get(_t.because).lower() + ' '\
								 + _t.get(_t.theyThink).lower() + ': '  + confrontation

		return _t.get(_t.sentencesOpenersForArguments[0]) + ': ' + opinion + '.<br><br>' + confrontation_text \
	...
	def get_text_for_status_one_in_confrontation(self, premise, conclusion, relation, supportive, attack,
		...
		if attack == 'undermine':
			...
			if reply_for_argument:
				...
				confrontation_text = _t.get(_t.otherParticipantsAcceptBut) + ' ' \
