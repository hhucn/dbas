...
def flag_statement_for_merge_or_split(key: str, pgroup: PremiseGroup, text_values: list(), db_user: User, tn: Translator) -> dict():
    ...
    # was this already flagged?
    flag_status = QueueAdapter(db_user=db_user).is_element_flagged(premisegroup_uid=pgroup.uid)
    if flag_status:
        logger('FlagingHelper', 'already flagged')
        if flag_status == FlaggedBy.user:
            info = tn.get(_.alreadyFlaggedByYou)
...
def flag_pgroup_for_merge_or_split(key: str, pgroup: PremiseGroup, db_user: User, tn: Translator) -> dict():
    ...
    :return: success, info
    """
    logger('FlagingHelper', f'Flag pgroup {pgroup.uid} for a {key}')
...
def __add_delete_review(argument_uid, statement_uid, user_uid, reason_uid):
    ...
    :return: None
    """
    logger('FlagingHelper', f'Flag argument/statement {argument_uid}/{statement_uid} by user {user_uid} for delete')
