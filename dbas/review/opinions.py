...
from dbas.database.discussion_model import User, LastReviewerDelete, LastReviewerEdit, LastReviewerOptimization, \
    LastReviewerDuplicate, LastReviewerSplit, LastReviewerMerge, ReviewEdit, ReviewEditValue, ReviewMergeValues, \
    Premise, Issue, Statement, PremiseGroup, Argument, PremiseGroupMerged, StatementReplacementsByPremiseGroupMerge, \
    ReviewSplitValues, PremiseGroupSplitted, ArgumentsAddedByPremiseGroupSplit, \
    StatementReplacementsByPremiseGroupSplit, RevokedDuplicate
from dbas.handler.statements import correct_statement, set_statement
...
def add_review_opinion_for_delete(db_user, main_page, db_review, should_delete, _t):
    ...
    logger('review.opinions', 'result ' + str(count_of_keep) + ':' + str(count_of_delete))
...
def add_review_opinion_for_optimization(db_user, main_page, db_review, should_optimized, new_data, _t):
    ...
    :return: String
    """
    logger('review.opinions', 'main ' + str(db_review.uid) + ', optimize ' + str(should_optimized))
    # add new review
...
def add_review_opinion_for_duplicate(db_user, main_page, db_review, is_duplicate, _t):
    ...
    :return: String
    """
    logger('review.opinions', 'main {}, duplicate {}'.format(db_review.uid, is_duplicate))
    db_user_created_flag = DBDiscussionSession.query(User).get(db_review.detector_uid)
    # add new vote
    ...
    logger('review.opinions', 'result ' + str(count_of_keep) + ':' + str(count_of_reset))

    # do we reached any limit?
    reached_max = max(count_of_keep, count_of_reset) >= max_votes
    if reached_max:
        if count_of_reset > count_of_keep:  # disable the flagged part
            logger('review.opinions', 'max reached / bend for review {}'.format(db_review.uid))
            ...
            logger('review.opinions', 'max reached / forget about review {}'.format(db_review.uid))
        ...
        logger('review.opinions', 'vote says forget about review {}'.format(db_review.uid))
        add_rep, broke_limit = add_reputation_for(db_user_created_flag, rep_reason_bad_duplicate)
        db_review.set_executed(True)
        db_review.update_timestamp()

    elif count_of_reset - count_of_keep >= min_difference:  # disable the flagged part
        logger('review.opinions', 'vote says bend for review {}'.format(db_review.uid))
        __bend_objects_of_duplicate_review(db_review)
        add_rep, broke_limit = add_reputation_for(db_user_created_flag, rep_reason_success_duplicate)
        db_review.set_executed(True)
        db_review.update_timestamp()
...
def add_review_opinion_for_split(db_user, main_page, db_review, should_split, _t):
    ...
    if reached_max:
        if count_of_keep > count_of_reset:  # split pgroup
            logger('review.opinions', 'max reached for review {}'.format(db_review.uid))
        ...
        logger('review.opinions', 'vote says split for review {}'.format(db_review.uid))
        __split_premisegroup(db_review)
        add_rep, broke_limit = add_reputation_for(db_user_created_flag, rep_reason_success_flag)
        db_review.set_executed(True)
        db_review.update_timestamp()
...
def add_review_opinion_for_merge(db_user, main_page, db_review, should_merge, _t):
    ...
    if reached_max:
        ...
        logger('review.opinions', 'vote says merge for review {}'.format(db_review.uid))
        __merge_premisegroup(db_review)
        add_rep, broke_limit = add_reputation_for(db_user_created_flag, rep_reason_success_flag)
        db_review.set_executed(True)
        db_review.update_timestamp()
...
def __add_vote_for(user, review, is_okay, review_type):
    ...
    logger('review.opinions', '{}, user {}'.format(msg, user.uid))
    ...
    if not already_voted:
        ...
        DBDiscussionSession.flush()
        transaction.commit()
        logger('review.opinions', 'vote added')
    else:
        logger('review.opinions', 'already voted')
...
def __proposal_for_the_element(db_review, data, db_user):
    ...
    for argument_uid in argument_dict:
        ...
        logger('review.opinions', 'New ReviewEdit with uid ' + str(db_review_edit.uid) + ' (argument)')
    ...
    for statement_uid in statement_dict:
        ...
        logger('review.opinions', 'New ReviewEdit with uid ' + str(db_review_edit.uid) + ' (statement)')
...
def __merge_premisegroup(review):
    ...
    if db_values:
        logger('review.opinions', 'merge given premisegroup with the mapped, new statements')
...
def __split_premisegroup(review):
    ...
    if db_values:
        logger('review.opinions', 'split given premisegroup into the mapped, new statements')
        db_statements = []
        for value in db_values:
...
def __bend_objects_of_duplicate_review(db_review):
    ...
    if db_dupl_statement.is_position and not db_orig_statement.is_position:
        logger('review.opinions', 'Duplicate is startpoint, but original one is not')
        DBDiscussionSession.add(
