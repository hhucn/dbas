...
class QueryHelper(object):
	...
	def get_ajax_arguments_by_pos(self, transaction, uid):
		"""
		...
		:return: dictioanry
		"""
		return_dict = self.get_arguments_for_justifications(uid)
	...
	def get_args_for_new_round(self, user, statement_id):
		...
		logger('QueryHelper', 'get_args_for_new_round', 'last statement is an argument: ' + (db_correction.text if db_correction else
		db_last_statement.text))
		return_dict['currentStatementText'] = db_correction.text if db_correction else db_last_statement.text
...
class DictionaryHelper(object):
	...
	def save_corrected_statement_row_in_dictionary(self, statement_row, is_argument, additional_key='', additional_value=''):
		...
		logger('DictionaryHelper', 'save_argument_row_in_dictionary', str(statement_row.uid) + ', ' + statement_row.text + ', ' + str(statement_row.date) + ', ' + str(statement_row.weight) + ', ' + str(statement_row.author))
		dict = {}
		dict['uid'] = str(statement_row.uid)
		dict['text'] = db_correction.text if db_correction else statement_row.text
		dict['date'] = str(statement_row.date)
		dict['weight'] = str(statement_row.weight)
