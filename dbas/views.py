...
class Dbas(object):
	...
	def fuzzy_search(self):
		...
		try:
			...
			sm = FuzzyStringMatcher()
			logger('fuzzy_search', 'main', 'value: ' + str(value) + ', mode: ' + str(mode) + ', issue: ' + str(issue))
			if mode == '0': # start statement
				return_dict = sm.get_fuzzy_string_for_start(value, issue, True)
			elif mode == '1': # edit statement popup
				statement_uid = self.request.params['extra']
				return_dict = sm.get_fuzzy_string_for_edits(value, statement_uid, issue)
			elif mode == '2':  # start premise
				return_dict = sm.get_fuzzy_string_for_start(value, issue, False)
			elif mode == '3':  # adding reasons
				return_dict = sm.get_fuzzy_string_for_reasons(value, issue)
			else:
