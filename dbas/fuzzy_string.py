...
class FuzzySearch(object):
	def get_fuzzy_string_for_start(self, value, issue, isStatement):
		...
		:return:
		"""
		logger('FuzzySearch', 'get_fuzzy_string_for_start', 'string: ' + value + ', isStatement: ' + str(isStatement))
		...
		for index, statement in enumerate(db_statements):
			...
			# logger('FuzzySearch', 'get_fuzzy_string_for_start', 'current db_textvalue ' + db_textvalue.textversions.content.lower())
			if value.lower() in db_textvalue.textversions.content.lower():
				...
				logger('FuzzySearch', 'get_fuzzy_string_for_start', 'lev: ' + str(lev) + ', value: ' + value.lower() + ' in: ' +  db_textvalue.textversions.content)
		...
		logger('FuzzySearch', 'get_fuzzy_string_for_start', 'dictionary length: ' + str(len(return_dict.keys())))
	...
	def get_fuzzy_string_for_edits(self, value, statement_uid, issue):
		...
		:return:
		"""
		logger('FuzzySearch', 'get_fuzzy_string_for_edits', 'string: ' + value + ', statement uid: ' + str(statement_uid))
		...
		for index, textversion in enumerate(db_textversions):
			if value.lower() in textversion.content.lower():
				...
				logger('FuzzySearch', 'get_fuzzy_string_for_edits', 'lev: ' + str(lev) + ', value: ' + value.lower() + ' in: ' + textversion.content.lower())
				if lev < 10:
					lev = '0000' + str(lev)
				elif lev < 100:
					lev = '000' + str(lev)
		...
		logger('FuzzySearch', 'get_fuzzy_string_for_edits', 'dictionary length: ' + str(len(return_dict.keys())))
	...
	def get_fuzzy_string_for_reasons(self, value, issue):
		...
		:return:
		"""
		logger('FuzzySearch', 'get_fuzzy_string_for_reasons', 'string: ' + value + ', issue: ' + str(issue))
		...
		logger('FuzzySearch', 'get_fuzzy_string_for_reasons', 'dictionary length: ' + str(len(return_dict.keys())))
