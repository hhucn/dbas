...
These migrations are saved in so called _revisions_ inside the `db-migrations` directory.
...
You may have to check if there was any error while generating. (Example: The generation was faulty, because the database had a circular dependency.)
Test your new revision by up- and downgrading the database with: `alembic upgrade +1` and `alembic downgrade -1`.
Upgrade to the latest revision with `alembic upgrade head`.

Setup the database for migration
--------------------------------
_alembic_ creates a table with just the current revision version as content.
To set this initial version use `alembic stamp 2a4bc7c8ff38`.
Revision `2a4bc7c8ff38` is the state of the _discussion_ database, as the migrations are introduced.
